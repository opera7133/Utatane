...
- モジュールカタログに詳細パネルを追加し、原作・ライセンス・導入手順・制約をアプリ内で確認できるようにした。導入済み版と更新版を区別し、初期設定では既存ゴーストが使うSHIORIを選択して、導入済みの最新版を再取得しない。
- 追加したゴーストに必要なSHIORI・SAORIがカタログにあれば導入を案内し、成功後に起動する。Wineを設定している場合はWindows DLLでの起動も選べる。
- 上級者向け設定から署名付きモジュールカタログのURLと公開鍵を変更できるようにした。
- ダブルクリックからSHIORI応答・再生開始までの所要時間をデバッグログに記録し、反応の遅れを調べやすくした。
- PNAマスク付きサーフェスの合成を高速化し、クリック時の表情切替で台詞が遅れる問題を修正した。
- SHIORIが終端記号だけを返した場合に空の会話で再生中の台詞を消さず、`OnTranslate`でも元の会話を保持するようにした。
- 華和梨の配布版dylibをゴースト同梱・共通導入から優先して読み込むようにした。
- 灯の配布版dylibを優先して読み込み、変数をゴーストの外へ保存できるようにした。
- 結奈の配布版dylibを優先して読み込み、状態をゴーストの外に保存できるようにした。
- 翡翠の配布版dylibを優先して読み込み、状態をゴーストの外に保存できるようにした。
- 忍の配布版dylibを優先して読み込み、SAORIを共通導入先から呼び出せるようにした。

- 署名付き正式モジュールカタログを標準の接続先に設定した。

- ese-shioriのdylibをゴースト同梱・カタログ共通導入から読み込めるようにした。未導入時は従来の内蔵実装を使用する

- 偽栞のdylibをゴースト同梱・カタログ共通導入から読み込めるようにした。未導入時は従来の内蔵実装を使用する

- 署名付きモジュールカタログの一覧・検索・導入画面を追加し、設定のSHIORI欄からも導入できるようにした。初回のSHIORI選択ではYAYAと里々を必須にした

- SAORIの`filename.dll`指定で、同じフォルダの`filename.dylib`、`libfilename.dylib`、共通導入版の順に読み込むようにした。wmoveを含むネイティブSAORIはゴーストごとの別プロセスで実行する

- pastaのmacOS版を同梱または共通導入して使えるようにした。ゴーストごとのプロセス分離と、破損した同梱版からの自動切り替えに対応した

- 配布版美坂のファイル名を`libmisaka.dylib`に統一し、ゴースト直下の同梱dylibを優先するようにしました。

- 通常のmacOS用SHIORIをゴースト・プラグインごとに別プロセスで実行し、複数起動時の状態混在を防ぐようにした。minatoの同梱版・共通導入版に対応した

- 同梱SHIORIの破損や関数の欠落を検出した場合に、同じSHIORIの共通導入版へ切り替える設定を追加した（美坂・kagari・aosora）。設定で無効にできる

- Native SHIORIの開発資料とビルド処理をutatane-modulesへ集約し、本体のビルドから呼び出すようにした

- 配布版の美坂dylibをゴーストの`shiori.macos`やプラグインの`filename.macos`から読み込み、共通導入版と同じ保存先・SAORIを使えるようにした
- 美坂製プラグインも外部モジュールを使い、変数をApplication Supportへ保存するようにした。従来の保存ファイルは残して引き継ぐ
- プラグイン再読込では旧セッションの終了を待ってから作り直し、終了後に届いた応答で古いプラグインが復活しないようにした
- 美坂のSHIORIを外部ライブラリから読み込めるようにした。ゴーストごとの保存先とSAORIを引き継ぎ、環境変数で明示したライブラリの読み込み失敗をエラーとして扱う
...
- サーフェス画像のキャッシュにキャラクターごとの上限（推定16MiB・128件）を設け、使われていない画像から解放するようにした。画像合成の共通化と、終了・再読込時の描画資源の解放も行った
- 発話履歴で同じサムネイルを共有し、画像の読み直しを減らした。履歴更新をまとめ、閉じた履歴画面の描画処理を解放することで、履歴が増えた時の文字送りの負荷を抑えた
- 呼び出しゴーストの休止中はアニメーション・音声・ファイル監視を止め、不要な画像キャッシュを解放するようにした。復帰時は休止時間を除いて再開し、保留中のSHIORI通知・応答を処理する
- HTTPの進捗・ストリーミング通知を順番に処理し、タスクや応答データの重複保持を減らした。ウインドウモードのゴースト切り替えで、古いEscapeキー処理が残る問題も修正した

### 表示・配置の修正

- 発話履歴のサムネイルが透明になる不具合を修正し、`icon.rect`や画像上部の切り抜き方向も修正した
- 起動時にシェルが自由配置を指定していても、ゴーストごとの「画面下に固定」設定を優先するようにした
- `balloonoffset`の変更・解除、会話終了、再読込で、利用者がドラッグしたバルーンの移動分を保持するようにした。位置初期化・プリセット復元にも反映する
- `dumpsurface`で負座標を含む合成画像を書き出せるようにし、「ゼロ位置切り出し」指定時だけ負座標部分を切り落とすようにした。表示用キャッシュと分離し、極端な合成範囲の拡張も制限した

### SHIORI・SakuraScript互換性

- `OnBalloonBreak`・`OnSSTPBreak`・消滅キャンセルの中断位置を、タグやエスケープを含むスクリプト先頭からの文字数で通知するようにした。話者・翻訳後のスクリプト・埋め込み応答の位置も追跡する
- SSTPの中断対象を再生中の会話から取得し、`nobreak`の予約があっても正しい対象を通知するようにした。会話の置き換え・キャンセル時には残った予約を破棄する
- SHIORIの起動・終了をNOTIFYで送り、ゴースト／SHIORI再読込時に`Reference0=reload`を渡すようにした。NOTIFYの返答では発話を開始しない
- 起動時の画面情報・着せ替え一覧通知を追加した。ウインドウモードでは、その表示領域の変更を対象ゴーストへ`OnDisplayChange`／`OnDisplayChangeEx`で通知する
- 着せ替えの複数変更は最後だけGET、それ以前はNOTIFYで送り、100件以上は全状態の通知にまとめるようにした。メニュー・スクリプト操作への返答も再生する
- `OnSurfaceChange`をNOTIFYへ修正し、キャラクターIDと画像サイズを追加した。バルーン情報には追加キャラクターの画像番号も含めるようにした
...
- 署名付きモジュールカタログからSHIORI・SAORIを共通導入する処理を追加した。HTTPで取得した一覧のEd25519署名、ZIPのサイズ・SHA-256、展開後のファイル一覧と各ハッシュを検証する
...
[Unreleased]: https://github.com/opera7133/Utatane/compare/v0.2.9...HEAD
