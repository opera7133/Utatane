...
- 署名付きのSHIORI・SAORIカタログを追加。初回起動時に既存ゴーストで使うモジュールを選べるようにし、YAYAと里々は必須にした
- 初回のモジュール導入が終わるまで、既存ゴーストの自動起動を待つようにした
- 新しいゴーストに必要なmacOS用モジュールがない場合はダウンロードを案内する。Wineで実行できる環境ではWindows DLLでの起動も選べる
- カタログ内で概要・原作・ライセンス・導入手順を確認できるようにした。SHIORI設定では、別の署名付きカタログのURLと公開鍵も指定できる

### SHIORI・SAORI

- ゴースト同梱のdylibを共通導入版より優先し、読み込み不能な同梱版からの切り替えを設定で選べるようにした。通常のmacOS用SHIORIはゴーストごとの別プロセスで実行する
- minato・pasta・華和梨と、独自実装の美坂・偽栞・似非shiori・結奈・翡翠・忍・灯の配布版dylibに対応した。既存のゴーストでも対応するWindows DLL名から利用できる
- SAORIは`filename.dll`に対し、同じフォルダの`filename.dylib`・`libfilename.dylib`、共通導入版の順に探すようにした。内蔵SAORIの配布版も利用できる
- 移植済みのSHIORI・SAORI実装とkagariの同梱をやめ、対応するdylibをカタログまたはゴーストから読み込むようにした。FIRST専用実装は残した
- 状態保存先とSAORIの共通導入先はモジュール共通の設定として渡し、保存ファイル名は各モジュールが管理するようにした
- 美坂の変数・ゴーストごとの状態とSAORI接続を配布版へ引き継ぎ、プラグインにも同じ読み込み経路を適用した

### 会話・表示

- SHIORIの空応答や`OnTranslate`で会話が消える問題を修正した
- PNAマスク付き画像の合成と描画済みサーフェスの再利用を改善し、表情切替時の待ち時間を短縮した
- ダブルクリックからSHIORI応答・台詞開始までの所要時間をデバッグログに記録するようにした
