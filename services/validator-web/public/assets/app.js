...
const executionLabels = {
  builtIn: "Utatane内蔵",
  bundledNativeModule: "同梱のネイティブモジュール",
  ...
  try {
    for (let attempt = 0; attempt <= maximumBusyRetries; attempt += 1) {
      ...
      if (!response.ok || !payload?.ok) {
        ...
        throw new Error(
          payload?.error?.message || "検査結果を受け取れませんでした。",
        );
      }
...
function retryAfterSeconds(value) {
  ...
  return Number.isFinite(seconds)
    ? Math.min(30, Math.max(1, Math.round(seconds)))
    : 5;
}
...
function renderReport(report, requestId) {
  ...
  document.querySelector("#ghost-name").textContent =
    report.ghostName || "取得できませんでした";
  ...
  document.querySelector("#shiori-name").textContent =
    assessment?.displayName || report.shiori || "判定できませんでした";
  ...
  const detailRows = [
    [
      "モジュール",
      assessment?.macOSModuleFilename || assessment?.declaredModuleFilename,
    ],
    [
      "実行方式",
      executionLabels[assessment?.execution] || assessment?.execution,
    ],
    [
      "追加要件",
      runtimeLabels[assessment?.runtimeRequirement] ||
        assessment?.runtimeRequirement,
    ],
  ].filter(([, value]) => value);
  const details = document.querySelector("#shiori-details");
  details.replaceChildren(
    ...detailRows.flatMap(([term, value]) => {
