...
struct NativeKawariSessionTests {
    ...
    @Test
    func `native textcopy2 SAORI passes text to the copy service`() {
        let recorder = TextCopyRecorder()
        let marker = "Utatane textcopy2 \(UUID().uuidString)"
        let request = "EXECUTE SAORI/1.0\r\n"
            + "Charset: Shift_JIS\r\n"
            + "Argument0: \(marker)\r\n"
            + "Argument1: 1\r\n\r\n"
        let response = nativeTextCopySaoriResponse(for: request) { recorder.value = $0 }

        #expect(response.contains("Result: \(marker)"))
        #expect(recorder.value == marker)
    }
...
private final class TextCopyRecorder: @unchecked Sendable {
