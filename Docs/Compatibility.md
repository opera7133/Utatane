...
| ユグドラシルチェリィ | SATORI | 起動、2キャラクター、シェル切り替えを確認。起動・再読込時の表示、まばたきの重なり、HTTP更新失敗を修正後、利用者確認 | 0.1.1でダブルクリックメニューの「閉じる」が効かない。`ukastream.dll`を使うルーレット |
...
| 偽さくら Rebirth 2.008 | ese-shiori + ParticleMakoto | 独自辞書の復号とese-shioriの評価処理、内部会話タイマー、他ゴースト検索・会話転送、辞書ファイル保存、ParticleMakotoをネイティブ実装。起動、定期会話、初回名前入力と選択肢、ダブルクリックメニュー、単語学習と再読込をテスト済み | Materiaニュースに依存する`READNEWS`、Neuron・Synapses学習グラフ、長時間の状態変化は未確認 |
