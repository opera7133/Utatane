...
- キャラクターへNARファイルをドラッグ＆ドロップする。
- 右クリックの「機能」→「コンテンツ管理」→「NARをインストール…」からファイルを選ぶ。
- Finderの「このアプリケーションで開く」でUtataneを選ぶ。
...
上部で種類を切り替え、検索欄で絞り込みます。左の一覧から項目を選ぶと、右に詳細と操作ボタンが表示されます。画像の一覧やフォルダは撮影環境の例です。

ゴースト、シェル、バルーン、ヘッドライン、プラグインを種類ごとに表示し、名前を検索できます。項目に応じて切り替え、ネットワーク更新、READMEや配布元の表示、Finderでの表示、アンインストールを利用できます。
