...
選択された`filename`または`filename.macos`が`.dylib`、`.so`、`.bundle`の場合は、まず一般的な伺かモジュールと同じexport名を探します。既存のオープンソースプラグインは、Windows固有処理とメモリ受け渡し部分だけを移植し、プラグイン本体のイベント処理を保ったままmacOS用`cdylib`としてビルドできます。
