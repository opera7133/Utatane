...
SHIOLINKは外部SHIORIそのものではなく接続方式として登録し、利用者が実行ファイルを用意する扱いです。似非shioriはese-shioriの別名、偽栞は`niseshiori.dll`を使う別のSHIORIとして区別します。どちらも専用のネイティブ実装を優先します。
...
偽さくら Rebirth 2.008を使ったテストでは、起動、定期会話、OnFirstBootの名前入力と選択肢遷移、ダブルクリックメニューに加え、入力ボックスからの単語学習と終了後の再読込までWineなしで確認しています。古いMateria互換の相方サーフェス番号も現行の番号へ変換します。元DLLを参照実装として組み込み関数の応答とファイル副作用を照合し、Materia形式の`[SAKURA]`ニュースを順番に読む`READNEWS`と既読位置の保存にも対応しています。

## ネイティブ偽栞

`niseshiori.dll`と`ai*.txt`または`ai*.dtx`を持つゴーストでは、DLLより`UtataneNiseShioriNative`を優先します。Shift_JIS、UTF-8、EUC-KR、EUC-JPの辞書と偽栞の暗号化形式を読み込み、単語・分類、イベント、条件、ランダムトーク、ニュース、リソース文字列を評価します。

メタ文字列、`\\set`の四則演算、`\\ns_st`、名前入力、ジャンプ、単語学習、見切れ・重なりカウンタ、定期トークを処理し、ユーザ名・変数・学習語・ニュース位置はゴースト本体ではなくApplication Supportへ保存します。偽栞1.0.20付属の平文サンプル辞書で起動応答を確認しています。

単語チェインは文中キーワードによる候補選択まで対応しています。原版の単語間チェイン、`\\ft`による文章分解、TEACHの対話全体、ランダムなニュース発話、古い本体固有のCPU・Windows環境変数はまだ完全互換ではありません。

`eseai.ini`のNeuron・Synapsesは辞書内で使う品詞タグ名の設定です。以前900要素の学習グラフと記載していた`ese_pop.dat`は、実際には`$PUSH`／`$POP`の100〜999番スロットを保存する900行のファイルでした。ネイティブ版は同じスタックをJSON状態へ保存します。原版独自の文章分解・再構成アルゴリズムとの完全一致までは保証しません。
