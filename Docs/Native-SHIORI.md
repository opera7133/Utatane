...
状態は配布物の`ghost/master`へ書かず、Utataneのゴースト別データ領域へJSONで保存します。偽さくら Rebirth 2.008を使ったテストでは、起動、定期会話、OnFirstBootの名前入力、ダブルクリックメニューに加え、入力ボックスからの単語学習と終了後の再読込までWineなしで確認しています。ニュース取得、元DLLのNeuron・Synapses学習グラフ、未使用命令を含む任意のese-shioriゴーストとの完全互換は未確認です。
