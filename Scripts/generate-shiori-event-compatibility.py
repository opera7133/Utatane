...
def merge_audited_rows(existing: str, generated: str, ukadoc_file: Path) -> str:
    generated_rows: dict[str, str] = {}
    for line in generated.splitlines():
        match = re.match(r"\| \[`([^`]+)`\]", line)
        if match and AUDITED_EVENTS.get(match.group(1), (None,))[0] == "✅":
            generated_rows[match.group(1)] = line
    ...
    for line in existing.splitlines():
        ...
        if match:
            ...
            if event_id in generated_rows:
                line = generated_rows[event_id]
