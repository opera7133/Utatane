...
const html = fs.readFileSync(path.join(__dirname, '../website/src/components/marketing/HomePage.astro'), 'utf8');
const css = fs.readFileSync(path.join(__dirname, '../website/src/styles/marketing.css'), 'utf8');
const script = fs.readFileSync(path.join(__dirname, '../website/src/scripts/marketing.js'), 'utf8');
...
(async()=>{
  ...
  assert.match(deploy,/mirror --reverse --verbose --parallel=4[^\n]+website\/dist "\$DEPLOY_TARGET"/);
  assert.ok(deploy.includes('bun run check && bun run build && bun run test'));
  assert.ok(deploy.includes('- "Docs/**"'));
  assert.ok(!deploy.includes('put -O'));
  assert.ok(deploy.includes('[[ "$WEBSITE_DEPLOY_PATH" != "/" ]]'));
  console.log('PASS: reduced motion and generated-site deployment entries');
})();
